...
control 'docker-4.3' do
  ...
  ref 'distroless', url: 'https://github.com/GoogleContainerTools/distroless'

  describe 'docker-4.3' do
    skip 'build distroless or minimal base images (https://github.com/GoogleContainerTools/distroless, https://alpinelinux.org)'
  end
end
...
control 'docker-4.7' do
  ...
  describe docker.images.where { repository == CONTAINER_NAME && tag == CONTAINER_TAG } do
    # hadolint checks for unpinned packges. Update/upgrade not permitted.
    describe command("docker --no-trunc history #{id}| grep -e 'update'") do
      its('stdout') { should eq '' }
    end
    describe command("docker --no-trunc history #{id}| grep -e 'upgrade'") do
      its('stdout') { should eq '' }
    end
  end
end
